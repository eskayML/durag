...
def memory_instance():
    with (
        patch("durag.utils.factory.EmbedderFactory") as mock_embedder,
        patch("durag.memory.main.VectorStoreFactory") as mock_vector_store,
        patch("durag.utils.factory.LlmFactory") as mock_llm,
        patch("durag.memory.telemetry.capture_event"),
    ):
...
def test_add(memory_instance):
    ...
    memory_instance._add_to_vector_store.assert_called_once_with(
        [{"role": "user", "content": "Test message"}], {"user_id": "test_user"}, {"user_id": "test_user"}, False, prompt=None
    )
...
def test_search(memory_instance):
    ...
    with patch("durag.memory.main.lemmatize_for_bm25", return_value="test query"), \
         patch("durag.memory.main.extract_entities", return_value=[]):
...
def test_no_telemetry_vector_store_when_disabled():
    ...
    with (
        patch("durag.memory.main.DURAG_TELEMETRY", False),
        patch("durag.utils.factory.EmbedderFactory") as mock_embedder,
        patch("durag.memory.main.VectorStoreFactory") as mock_vector_store,
        patch("durag.utils.factory.LlmFactory") as mock_llm,
        patch("durag.memory.telemetry.capture_event"),
...
def test_telemetry_vector_store_created_when_enabled():
    ...
    with (
        patch("durag.memory.main.DURAG_TELEMETRY", True),
        patch("durag.utils.factory.EmbedderFactory") as mock_embedder,
        patch("durag.memory.main.VectorStoreFactory") as mock_vector_store,
        patch("durag.utils.factory.LlmFactory") as mock_llm,
        patch("durag.memory.telemetry.capture_event"),
...
class TestSearchParamValidation:
    ...
    def test_search_accepts_threshold_zero(self, memory_instance):
        ...
        with patch("durag.memory.main.lemmatize_for_bm25", return_value="test"), \
             patch("durag.memory.main.extract_entities", return_value=[]):
