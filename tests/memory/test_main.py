...
from durag.memory.main import AsyncMemory, Memory
...
def _setup_mocks(mocker):
    ...
    mocker.patch("durag.utils.factory.EmbedderFactory.create", mock_embedder)

    mock_vector_store = mocker.MagicMock()
    mock_vector_store.return_value.search.return_value = []
    mocker.patch(
        "durag.utils.factory.VectorStoreFactory.create", side_effect=[mock_vector_store.return_value, mocker.MagicMock()]
    )

    mock_llm = mocker.MagicMock()
    mocker.patch("durag.utils.factory.LlmFactory.create", mock_llm)

    mocker.patch("durag.memory.storage.SQLiteManager", mocker.MagicMock())
...
class TestAddToVectorStoreErrors:
    ...
    def test_empty_llm_response_fact_extraction(self, mocker, mock_memory, caplog):
        ...
        mocker.patch("durag.memory.main.capture_event")
...
class TestAsyncAddToVectorStoreErrors:
    ...
    async def test_async_empty_llm_response_fact_extraction(self, mock_async_memory, caplog, mocker):
        ...
        mocker.patch("durag.utils.factory.EmbedderFactory.create", return_value=MagicMock())
    ...
    async def test_async_empty_llm_response_memory_actions(self, mock_async_memory, caplog, mocker):
        ...
        mocker.patch("durag.memory.main.capture_event", mock_capture_event)
...
def _build_memory_instance(mocker, memory_cls):
    _setup_mocks(mocker)
    mocker.patch("durag.memory.main.SQLiteManager", mocker.MagicMock())
    mocker.patch("durag.memory.main.DURAG_TELEMETRY", False)
    memory = memory_cls()
    memory.config = mocker.MagicMock()
    memory.config.custom_instructions = None
    memory.config.custom_update_memory_prompt = None
