...
class TestOpenSearchDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = patch("durag.vector_stores.opensearch.OpenSearch", return_value=self.client_mock)
    ...
    @patch("durag.vector_stores.opensearch.logger")
    ...
    def test_init_with_http_auth(self):
        ...
        with patch("durag.vector_stores.opensearch.OpenSearch") as mock_opensearch:
            OpenSearchDB(
                host="localhost",
                port=9200,
                collection_name="test_collection",
...
@patch("durag.utils.factory.EmbedderFactory.create")
@patch("durag.utils.factory.VectorStoreFactory.create")
@patch("durag.utils.factory.LlmFactory.create")
@patch("durag.memory.storage.SQLiteManager")
def test_safe_deepcopy_config_handles_opensearch_auth(
    ...
    from durag.memory.main import _safe_deepcopy_config
...
def test_memory_initialization_opensearch_aws_auth(
    ...
    config.vector_store.config = MockOpenSearchConfig(collection_name="durag_test", include_auth=True)
