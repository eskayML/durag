...
@pytest.fixture
def tmp_durag_dir(tmp_path, monkeypatch):
    """Point the durag setup module at a tempdir for the duration of the test."""
    monkeypatch.setenv("DURAG_DIR", str(tmp_path))
    # Reload setup so module-level durag_dir picks up the env var.
    import durag.memory.setup as setup_module

    importlib.reload(setup_module)
    yield tmp_path
    # Restore default state.
    monkeypatch.delenv("DURAG_DIR", raising=False)
    importlib.reload(setup_module)
...
class TestSetupConfigIdempotent:
    def test_creates_config_when_missing(self, tmp_durag_dir):
        import durag.memory.setup as setup_module

        setup_module.setup_config()
        config = json.loads((tmp_durag_dir / "config.json").read_text())
        assert "user_id" in config and config["user_id"]

    def test_backfills_user_id_when_only_telemetry_present(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
    ...
    def test_does_not_overwrite_existing_user_id(self, tmp_durag_dir):
        import durag.memory.setup as setup_module

        _write_config(tmp_durag_dir, {"user_id": "existing-uuid"})
        setup_module.setup_config()
        config = json.loads((tmp_durag_dir / "config.json").read_text())
        assert config["user_id"] == "existing-uuid"

    def test_handles_malformed_json(self, tmp_durag_dir):
        import durag.memory.setup as setup_module

        (tmp_durag_dir / "config.json").write_text("{not json")
...
class TestReadAnonIds:
    def test_returns_oss_only(self, tmp_durag_dir):
        import durag.memory.setup as setup_module

        _write_config(tmp_durag_dir, {"user_id": "oss-uuid"})
        anon = setup_module.read_anon_ids()
        assert anon == {"oss": "oss-uuid", "cli": None, "aliased_pairs": []}

    def test_returns_cli_only(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
    ...
    def test_returns_both(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
    ...
    def test_no_config_returns_all_none(self, tmp_durag_dir):
        import durag.memory.setup as setup_module

        anon = setup_module.read_anon_ids()
        assert anon == {"oss": None, "cli": None, "aliased_pairs": []}

    def test_malformed_json_does_not_raise(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
...
class TestMarkAliased:
    def test_writes_aliased_pair_preserving_other_fields(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
    ...
    def test_creates_telemetry_section_when_missing(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
    ...
    def test_tracks_each_pair_independently(self, tmp_durag_dir):
        import durag.memory.setup as setup_module
...
class TestCaptureIdentify:
    def test_fires_identify_with_anon_distinct_id(self):
        import durag.memory.telemetry as telemetry_module

        with patch.object(telemetry_module, "DURAG_TELEMETRY", True):
            with patch("durag.memory.telemetry.Posthog") as mock_posthog_cls:
    ...
    def test_noop_when_telemetry_disabled(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY", False):
            at = telemetry_module.AnonymousTelemetry()
...
class TestMaybeAliasAnonToEmail:
    ...
    def test_fires_identify_for_oss_uuid(self):
        from durag.client import main as client_main
    ...
    def test_skips_when_telemetry_disabled(self):
        """When client_telemetry.posthog is None (DURAG_TELEMETRY=false), do nothing —
...
class TestEndToEndIdempotency:
    ...
    def test_second_call_is_noop_after_pair_marker_persisted(self, tmp_durag_dir):
