...
from durag import AsyncMemory, Memory
from durag.configs.llms.base import BaseLlmConfig
from durag.llms.vllm import VllmLLM


@pytest.fixture
def mock_vllm_client():
    with patch("durag.llms.vllm.OpenAI") as mock_openai:
        mock_client = Mock()
...
def create_mocked_memory():
    ...
    with patch('durag.utils.factory.LlmFactory.create') as mock_llm_factory, \
         patch('durag.utils.factory.EmbedderFactory.create') as mock_embedder_factory, \
         patch('durag.utils.factory.VectorStoreFactory.create') as mock_vector_factory, \
         patch('durag.memory.storage.SQLiteManager') as mock_sqlite:
...
async def test_async_thinking_tags_async():
    ...
    with patch('durag.memory.main.asyncio.to_thread', side_effect=mock_to_thread):
