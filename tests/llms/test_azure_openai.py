...
def mock_openai_client():
    with patch("durag.llms.azure_openai.AzureOpenAI") as mock_openai:
        mock_client = Mock()
...
def test_generate_with_http_proxies(default_headers):
    ...
    with (
        patch("durag.llms.azure_openai.AzureOpenAI") as mock_azure_openai,
        ...
        config = AzureOpenAIConfig(
            ...
            top_p=TOP_P,
            api_key="test",
            http_client_proxies="http://testproxy.durag.net:8000",
        ...
        mock_http_client.assert_called_once_with(proxies="http://testproxy.durag.net:8000")
...
def test_init_with_api_key(monkeypatch):
    ...
    with patch("durag.llms.azure_openai.AzureOpenAI") as mock_azure_openai:
        llm = AzureOpenAILLM(config)
        mock_azure_openai.assert_called_once_with(
...
def test_init_with_default_azure_credential(monkeypatch):
    ...
    with (
        patch("durag.llms.azure_openai.DefaultAzureCredential") as mock_cred,
        patch("durag.llms.azure_openai.get_bearer_token_provider") as mock_token_provider,
        patch("durag.llms.azure_openai.AzureOpenAI") as mock_azure_openai,
    ):
