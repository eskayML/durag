...
class TestTelemetryDisabled:
    ...
    def test_posthog_not_created_when_disabled(self):
        """Posthog() constructor should never be called when DURAG_TELEMETRY=False."""
        with patch.object(telemetry_module, "DURAG_TELEMETRY", False):
            with patch("durag.memory.telemetry.Posthog") as mock_posthog:
...
class TestTelemetryEnabled:
    ...
    def test_posthog_created_when_enabled(self):
        """Posthog() should be instantiated when DURAG_TELEMETRY=True."""
        with patch.object(telemetry_module, "DURAG_TELEMETRY", True):
            with patch("durag.memory.telemetry.Posthog") as mock_posthog:
                with patch("durag.memory.telemetry.get_or_create_user_id", return_value="test-user"):
                    at = telemetry_module.AnonymousTelemetry()
...
class TestAnonymousTelemetryClose:
    ...
    def test_close_sets_posthog_to_none(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY", True):
            with patch("durag.memory.telemetry.Posthog"):
                with patch("durag.memory.telemetry.get_or_create_user_id", return_value="u"):
                    at = telemetry_module.AnonymousTelemetry()
                    at.close()
                    assert at.posthog is None
...
class TestMemoryLifecycle:
    ...
    def _make_mock_memory(self):
        ...
        from durag.memory.main import Memory
...
class TestAsyncMemoryLifecycle:
    ...
    def _make_mock_async_memory(self):
        from durag.memory.main import AsyncMemory
...
class TestTelemetryEnvVar:
    """Verify the DURAG_TELEMETRY env var parsing logic."""
    ...
    def test_oss_capture_event_does_not_crash_memory_init(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY", True):
            ...
            with patch.object(telemetry_module, "_oss_telemetry_instance", mock_at):
                ...
                telemetry_module.capture_event("durag.init", mock_memory)
