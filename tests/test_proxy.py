...
def mock_openai_embedding_client():
    with patch("durag.embeddings.openai.OpenAI") as mock_openai:
        mock_client = Mock()
...
def mock_openai_llm_client():
    with patch("durag.llms.openai.OpenAI") as mock_openai:
        mock_client = Mock()
        mock_openai.return_value = mock_client
        yield mock_client
...
@pytest.fixture
def mock_litellm():
    with patch("durag.proxy.main.litellm") as mock:
        yield mock


def test_durag_initialization_with_api_key(mock_openai_embedding_client, mock_openai_llm_client):
    durag = Durag()
    assert isinstance(durag.durag_client, Memory)
    assert isinstance(durag.chat, Chat)


def test_durag_initialization_with_config():
    config = {"some_config": "value"}
    with patch("durag.Memory.from_config") as mock_from_config:
        durag = Durag(config=config)
        mock_from_config.assert_called_once_with(config)
        assert isinstance(durag.chat, Chat)


def test_durag_initialization_without_params(mock_openai_embedding_client, mock_openai_llm_client):
    durag = Durag()
    assert isinstance(durag.durag_client, Memory)
    assert isinstance(durag.chat, Chat)
