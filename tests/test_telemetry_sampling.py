...
class TestSamplingBeforeSend:
    ...
    def test_lifecycle_event_passes_through_at_high_random(self):
        ...
        with patch("durag.memory.telemetry.random.random", return_value=0.999):
            for event in ("durag.init", "durag.reset", "durag._create_procedural_memory"):
    ...
    def test_hot_path_event_dropped_when_random_above_rate(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.1):
            with patch("durag.memory.telemetry.random.random", return_value=0.99):
                assert telemetry_module._sampling_before_send(self._msg("durag.add")) is None

    def test_hot_path_event_passes_when_random_below_rate(self):
        """At default rate 0.1, random=0.05 should pass the event through."""
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.1):
            with patch("durag.memory.telemetry.random.random", return_value=0.05):
                result = telemetry_module._sampling_before_send(self._msg("durag.add"))
                assert result is not None
                assert result["properties"]["sample_rate"] == 0.1

    def test_hot_path_annotates_sample_rate(self):
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.1):
            with patch("durag.memory.telemetry.random.random", return_value=0.0):
                result = telemetry_module._sampling_before_send(self._msg("durag.search"))
                assert result["properties"]["sample_rate"] == 0.1

    def test_lifecycle_annotates_sample_rate_one(self):
        result = telemetry_module._sampling_before_send(self._msg("durag.init"))
        assert result["properties"]["sample_rate"] == 1.0

    def test_rate_zero_drops_all_hot_path(self):
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.0):
    ...
    def test_rate_zero_still_passes_lifecycle(self):
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.0):
            with patch("durag.memory.telemetry.random.random", return_value=0.999):
                assert telemetry_module._sampling_before_send(self._msg("durag.init")) is not None

    def test_rate_one_passes_all_events(self):
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 1.0):
            with patch("durag.memory.telemetry.random.random", return_value=0.999):
                # 0.999 > 1.0 is False, so the gate never trips
                assert telemetry_module._sampling_before_send(self._msg("durag.add")) is not None
    ...
    def test_does_not_override_caller_supplied_sample_rate_for_hot_path(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY_SAMPLE_RATE", 0.1):
            with patch("durag.memory.telemetry.random.random", return_value=0.0):
                msg = self._msg("durag.add", {"sample_rate": 0.99, "other": "x"})
    ...
    def test_handles_missing_properties_dict(self):
        ...
        msg = {"event": "durag.init"}  # no properties
        result = telemetry_module._sampling_before_send(msg)
        assert result is not None
        assert result["properties"]["sample_rate"] == 1.0
    ...
    def test_oss_singleton_constructed_with_before_send_hook(self):
        ...
        with patch.object(telemetry_module, "DURAG_TELEMETRY", True):
            with patch("durag.memory.telemetry.Posthog") as mock_posthog_cls:
                with patch("durag.memory.telemetry.get_or_create_user_id", return_value="u"):
...
class TestSamplingDoesNotBreakExistingBehavior:
    ...
    def test_default_sample_rate_is_ten_percent(self):
        ...
        # We can't assert DURAG_TELEMETRY_SAMPLE_RATE directly because the user
