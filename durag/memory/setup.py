...
os.makedirs(durag_dir, exist_ok=True)

_logger = logging.getLogger(__name__)


def _config_path():
    return os.path.join(durag_dir, "config.json")


def _load_config():
    """Load ~/.durag/config.json, returning {} on missing/malformed file."""
    path = _config_path()
    ...
    try:
        ...
        _logger.debug("Failed to load durag config %s: %s", path, e)
        return {}


def _write_config(config):
    """Best-effort write of ~/.durag/config.json. Never raises."""
    path = _config_path()
    try:
        with open(path, "w") as f:
            json.dump(config, f, indent=4)
    except Exception as e:
        _logger.debug("Failed to write durag config %s: %s", path, e)


def setup_config():
    """Ensure ~/.durag/config.json exists with a top-level user_id.
...
def read_anon_ids():
    """Return anon IDs and alias markers from ~/.durag/config.json.
