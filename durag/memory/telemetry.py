...
DURAG_TELEMETRY = os.environ.get("DURAG_TELEMETRY", "True")
PROJECT_API_KEY = "phc_hgJkUVJFYtmaJqrvf6CYN67TIQ8yhXAkWzUn9AMU4yX"
HOST = "https://us.i.posthog.com"

if isinstance(DURAG_TELEMETRY, str):
    DURAG_TELEMETRY = DURAG_TELEMETRY.lower() in ("true", "1", "yes")

if not isinstance(DURAG_TELEMETRY, bool):
    raise ValueError("DURAG_TELEMETRY must be a boolean value.")
...
# Override via DURAG_TELEMETRY_SAMPLE_RATE env var.
_DEFAULT_SAMPLE_RATE = 0.1


def _parse_sample_rate(raw):
    """Parse DURAG_TELEMETRY_SAMPLE_RATE env var. Never raises."""
    try:
        value = float(raw)
    except (TypeError, ValueError):
        _logger.debug("DURAG_TELEMETRY_SAMPLE_RATE %r is not a number, defaulting to %s", raw, _DEFAULT_SAMPLE_RATE)
        return _DEFAULT_SAMPLE_RATE
    if not 0.0 <= value <= 1.0:
        _logger.debug("DURAG_TELEMETRY_SAMPLE_RATE %s out of [0.0, 1.0], defaulting to %s", value, _DEFAULT_SAMPLE_RATE)
        return _DEFAULT_SAMPLE_RATE
    return value


DURAG_TELEMETRY_SAMPLE_RATE = _parse_sample_rate(os.environ.get("DURAG_TELEMETRY_SAMPLE_RATE", str(_DEFAULT_SAMPLE_RATE)))

# Events that bypass sampling and always fire. Keep this set in sync with the
# event names passed to capture_event() in durag/memory/main.py.
# $identify is included so PostHog person-merging is never lost to sampling.
_LIFECYCLE_EVENTS = frozenset({"durag.init", "durag.reset", "durag._create_procedural_memory", "$identify"})
...
def _sampling_before_send(msg):
    ...
    if not is_lifecycle and random.random() >= DURAG_TELEMETRY_SAMPLE_RATE:
        return None

    # Annotate so PostHog dashboards can extrapolate true counts via 1/sample_rate.
    properties = msg.setdefault("properties", {})
    properties["sample_rate"] = 1.0 if is_lifecycle else DURAG_TELEMETRY_SAMPLE_RATE
    return msg
...
class AnonymousTelemetry:
    ...
    def capture_event(self, event_name, properties=None, user_email=None):
        ...
        properties = {
            ...
            "client_version": durag.__version__,
