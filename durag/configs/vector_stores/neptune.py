...
as a vector store backend for Durag's memory layer.
"""
...
class NeptuneAnalyticsConfig(BaseModel):
    ...
    for storing and retrieving memory embeddings in Durag.
    
    Attributes:
        collection_name (str): Name of the collection to store vectors. Defaults to "durag".
        endpoint (str): Neptune Analytics graph endpoint URL or Graph ID for the runtime.
    """
    collection_name: str = Field("durag", description="Default name for the collection")
