...
from durag.vector_stores.base import VectorStoreBase
...
class NeptuneAnalyticsVector(VectorStoreBase):
    """
    Neptune Analytics vector store implementation for Durag.
    ...
    _COLLECTION_PREFIX = "DURAG_VECTOR_"
    _FIELD_N = 'n'
    _FIELD_ID = '~id'
    ...
    def list_cols(self):
        """
        List all collections with the Durag prefix.
        
        Queries the Neptune Analytics schema to find all node labels that start
        with the Durag collection prefix.
